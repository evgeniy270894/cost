    ...
    <el-col :span="12" style="margin-top: 20px">
      <el-form :model="form" :rules="rules" ref="form" label-width="80px">
        ...
        <el-form-item class="btns">
          <el-button type="primary" @click="onHandleSubmit('form')">Сохранить</el-button>
...
export default {
  ...
  methods: {
    onHandleSubmit (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$store.commit('currentUserState/editCard', {
  ...
  created () {
    this.form.summ = vue.$store.getters['currentUserState/getCardById'](this.$route.params.id).money
    this.form.desc = vue.$store.getters['currentUserState/getCardById'](this.$route.params.id).desc
    this.form.type = vue.$store.getters['currentUserState/getCardById'](this.$route.params.id).type
  }
}
</script>
